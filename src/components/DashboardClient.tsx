...
export default function DashboardClient({ initialStats, todaysCount }: DashboardClientProps) {
    const { currentBalance } = usePensionStream(todaysCount, initialStats.totalStreamed);

    // Dynamic Net Worth: Pending Cash + Live Streamed Balance + Yield
    const liveNetWorth = initialStats.pending + currentBalance + initialStats.yieldGained;
        ...
        <main className="min-h-screen bg-[#09090b] selection:bg-emerald-500/30">
            ...
            <div className="w-full max-w-[1200px] mx-auto px-6 pb-20 pt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="md:col-span-2 bg-[#121212] border border-gray-800 rounded-3xl p-8 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <h2 className="text-gray-400 font-medium mb-1 flex items-center">
                                Total Net Worth
                                <InfoTooltip text="Sum of Realized Earnings + Pending Cash + Yield Generated." />
                            </h2>
                            <div className="text-5xl font-black text-white tracking-tight mb-6">
                                ${liveNetWorth.toFixed(2)}
                            </div>
                            <div className="flex gap-4">
                                <div className="bg-emerald-500/10 px-4 py-2 rounded-xl border border-emerald-500/20">
                                    <span className="block text-xs text-emerald-500 font-bold uppercase flex items-center gap-1">
                                        Streaming <InfoTooltip text="Money currently flowing into DeFi pools." />
                                    </span>
                                ...
                                <div className="bg-blue-500/10 px-4 py-2 rounded-xl border border-blue-500/20">
                                    <span className="block text-xs text-blue-500 font-bold uppercase flex items-center gap-1">
                                        Pending <InfoTooltip text="Cash kept in your pocket (99%)." />
                                    </span>
                                    <span className="text-lg font-bold text-white">${initialStats.pending.toFixed(2)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-br from-emerald-900/20 to-[#121212] border border-emerald-500/20 rounded-3xl p-8 flex flex-col justify-center relative overflow-hidden">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2 flex items-center">
                            Eco-Impact <InfoTooltip text="Your contributions to green DeFi protocols." />
                        </h3>
                ...
                <div className="flex flex-col gap-12">
                    ...
                    {/* Horizontal AI Feed */}
                    <AIInsightsFeed dailyDeliveries={todaysCount} currentBalance={currentBalance} totalStreamed={currentBalance} />
